...
export function getBendCommands(a: RGCoordinate, b: RGCoordinate, c: RGCoordinate, radiusSize: number): PathCommand[] {
  const bendSize = Math.min(distance(a, b) / 2, distance(b, c) / 2, radiusSize);
  const { x, y } = b;
...
export const createPathCommandsByPoints = (points: RGCoordinate[], borderRadius:number): PathCommand[] => {
  const pathCommands: PathCommand[] = [];
  points.forEach((p, i) => {
      if (i === 0) {
        pathCommands.push({ type: 'M', x: Math.round(p.x), y: Math.round(p.y) });
      } else if (i === points.length - 1) {
        const prevPoint = points[i - 1];
        let xDir = prevPoint.x === p.x ? 0 : (prevPoint.x < p.x ? -1 : 1);
        let yDir = prevPoint.y === p.y ? 0 : (prevPoint.y < p.y ? -1 : 1);
        if (points.length === 2) {
          xDir = 0;
          yDir = 0;
        }
        pathCommands.push({ type: 'l', dx: Math.round(p.x - prevPoint.x + borderRadius * xDir), dy: Math.round(p.y - prevPoint.y + borderRadius * yDir) });
      } else {
        const radiusPath = getBendCommands(points[i - 1], p, points[i + 1], borderRadius);
        // console.log('radiusPath:', points[i - 1], p, points[i + 1], radiusPath);
        pathCommands.push(...radiusPath);
      }
  });
  return pathCommands;
};
...
export const getJunctionPointDirection = (junctionPoint: RGJunctionPoint | undefined, x:number, y: number, isEnd: boolean, f_x: number, f_y: number, t_x: number, t_y: number) => {
  ...
  } else if (junctionPoint === 'horizontalLine') {
    if (!isEnd) {
      return f_y < t_y ? RGDirection.Bottom : RGDirection.Top;
    } else {
      return f_y > t_y ? RGDirection.Bottom : RGDirection.Top;
    }
  } else if (junctionPoint === 'verticalLine') {
    if (!isEnd) {
      return f_x < t_x ? RGDirection.Right : RGDirection.Left;
    } else {
      return f_x > t_x ? RGDirection.Right : RGDirection.Left;
    }
  }
